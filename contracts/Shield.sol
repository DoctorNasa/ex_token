...
contract Shield is Ownable {
  ...
  event SetxxxTokenPerBlock(uint256 indexed _xxxTokenPerBlock);
  ...
  event TransferFairLaunchOwnership(address newOwner);
  ...
  /// @dev Set xxxToken per Block on FLV1. Effect immediately on the next block.
  /// @param _xxxTokenPerBlock The new xxxTokenPerBlock
  function setxxxTokenPerBlock(uint256 _xxxTokenPerBlock) public onlyOwner {
    fairLaunchV1.setxxxTokenPerBlock(_xxxTokenPerBlock);
    emit SetxxxTokenPerBlock(_xxxTokenPerBlock);
  }
  ...
  /// @dev Maunally mint xxxToken warchest portion.
  ...
  function transferFairLaunchOwnership(address newOwner) public onlyOwner {
    fairLaunchV1.transferOwnership(newOwner);
    emit TransferFairLaunchOwnership(newOwner);
  }
}

//edit xxxtoken to name tokens