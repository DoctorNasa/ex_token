...
// xxxx Token with Governance.
contract xxxToken is ERC20("xxx Token", "xxx"), Ownable {
  ...
  function canUnlockAmount(address _account) public view returns (uint256) {
    // When block number less than startReleaseBlock, no xxxToken can be unlocked
    if (block.number < startReleaseBlock) {
      return 0;
    }
    // When block number more than endReleaseBlock, all locked xxxToken can be unlocked
    else if (block.number >= endReleaseBlock) {
      return _locks[_account];
    }
    // When block number is more than startReleaseBlock but less than endReleaseBlock,
    // some xxxToken can be released
    else
    {
  ...
  function unlock() public {
    require(_locks[msg.sender] > 0, "no locked xxxToken");
  ...
  // @dev move xxxToken with its locked funds to another account
  ...
  function delegateBySig(
    ...
    require(signatory != address(0), "xxxToken::delegateBySig: invalid signature");
    require(nonce == nonces[signatory]++, "xxxToken::delegateBySig: invalid nonce");
    require(now <= expiry, "xxxToken::delegateBySig: signature expired");
  ...
  function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
    require(blockNumber < block.number, "xxxToken::getPriorVotes: not yet determined");
  ...
  function _delegate(address delegator, address delegatee) internal {
    ...
    uint256 delegatorBalance = balanceOf(delegator); // balance of underlying xxxToken (not scaled);
  ...
  function _writeCheckpoint(
    ...
    uint32 blockNumber = safe32(block.number, "xxxToken::_writeCheckpoint: block number exceeds 32 bits");
...
interface IFairLaunch {
  ...
  function pendingxxxToken(uint256 _pid, address _user) external view returns (uint256);
...
// FairLaunch is a smart contract for distributing xxxToken by asking user to stake the ERC20-based token.
contract FairLaunch is IFairLaunch, Ownable {
  ...
  struct UserInfo {
    ...
    //
    // We do some fancy math here. Basically, any point in time, the amount of xxxToken
    // entitled to a user but is pending to be distributed is:
    //
    //   pending reward = (user.amount * pool.accxxxTokenPerShare) - user.rewardDebt
    //
    // Whenever a user deposits or withdraws Staking tokens to a pool. Here's what happens:
    //   1. The pool's `accxxxTokenPerShare` (and `lastRewardBlock`) gets updated.
  ...
  struct PoolInfo {
    ...
    uint256 allocPoint; // How many allocation points assigned to this pool. xxxToken to distribute per block.
    uint256 lastRewardBlock; // Last block number that xxxToken distribution occurs.
    uint256 accxxxTokenPerShare; // Accumulated xxxToken per share, times 1e12. See below.
    uint256 accxxxTokenPerShareTilBonusEnd; // Accumated xxxToken per share until Bonus End.
  }

  // The xxxToken TOKEN!
  xxxTokenToken public xxxToken;
  // Dev address.
  address public devaddr;
  // xxxToken tokens created per block.
  uint256 public xxxTokenPerBlock;
  // Bonus muliplier for early xxxToken makers.
  uint256 public bonusMultiplier;
  // Block number when bonus xxxToken period ends.
  ...
  // The block number when xxxToken mining starts.
  ...
  constructor(
    xxxTokenToken _xxxToken,
    address _devaddr,
    uint256 _xxxTokenPerBlock,
    uint256 _startBlock,
    ...
    xxxToken = _xxxToken;
    devaddr = _devaddr;
    xxxTokenPerBlock = _xxxTokenPerBlock;
  ...
  */
  ...
  function setxxxTokenPerBlock(uint256 _xxxTokenPerBlock) public onlyOwner {
    xxxTokenPerBlock = _xxxTokenPerBlock;
  }
  ...
  function addPool(
    ...
    poolInfo.push(
      PoolInfo({
        ...
        accxxxTokenPerShare: 0,
        accxxxTokenPerShareTilBonusEnd: 0
      })
    );
  }

  // Update the given pool's xxxToken allocation point. Can only be called by the owner.
  function setPool(
    uint256 _pid,
    uint256 _allocPoint,
    bool _withUpdate
  ...
  */
  ...
  function manualMint(address _to, uint256 _amount) public onlyOwner {
    xxxToken.manualMint(_to, _amount);
  }
  ...
  // View function to see pending xxxToken on frontend.
  function pendingxxxToken(uint256 _pid, address _user) external override view returns (uint256) {
    PoolInfo storage pool = poolInfo[_pid];
    UserInfo storage user = userInfo[_pid][_user];
    uint256 accxxxTokenPerShare = pool.accxxxTokenPerShare;
    uint256 lpSupply = IERC20(pool.stakeToken).balanceOf(address(this));
    if (block.number > pool.lastRewardBlock && lpSupply != 0) {
      uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
      uint256 xxxTokenReward = multiplier.mul(xxxTokenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
      accxxxTokenPerShare = accxxxTokenPerShare.add(xxxTokenReward.mul(1e12).div(lpSupply));
    }
    return user.amount.mul(accxxxTokenPerShare).div(1e12).sub(user.rewardDebt);
  }
  ...
  function updatePool(uint256 _pid) public override {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 xxxTokenReward = multiplier.mul(xxxTokenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    xxxToken.mint(devaddr, xxxTokenReward.div(10));
    xxxToken.mint(address(this), xxxTokenReward);
    pool.accxxxTokenPerShare = pool.accxxxTokenPerShare.add(xxxTokenReward.mul(1e12).div(lpSupply));
    // update accxxxTokenPerShareTilBonusEnd
    if (block.number <= bonusEndBlock) {
      xxxToken.lock(devaddr, xxxTokenReward.div(10).mul(bonusLockUpBps).div(10000));
      pool.accxxxTokenPerShareTilBonusEnd = pool.accxxxTokenPerShare;
    }
    if(block.number > bonusEndBlock && pool.lastRewardBlock < bonusEndBlock) {
      uint256 xxxTokenBonusPortion = bonusEndBlock.sub(pool.lastRewardBlock).mul(bonusMultiplier).mul(xxxTokenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
      xxxToken.lock(devaddr, xxxTokenBonusPortion.div(10).mul(bonusLockUpBps).div(10000));
      pool.accxxxTokenPerShareTilBonusEnd = pool.accxxxTokenPerShareTilBonusEnd.add(xxxTokenBonusPortion.mul(1e12).div(lpSupply));
    }
    pool.lastRewardBlock = block.number;
  }

  // Deposit Staking tokens to FairLaunchToken for xxxToken allocation.
  function deposit(address _for, uint256 _pid, uint256 _amount) public override {
    ...
    user.rewardDebt = user.amount.mul(pool.accxxxTokenPerShare).div(1e12);
    user.bonusDebt = user.amount.mul(pool.accxxxTokenPerShareTilBonusEnd).div(1e12);
  ...
  // Harvest xxxToken earn from the pool.
  ...
  function _harvest(address _to, uint256 _pid) internal {
    ...
    uint256 pending = user.amount.mul(pool.accxxxTokenPerShare).div(1e12).sub(user.rewardDebt);
    require(pending <= xxxToken.balanceOf(address(this)), "wtf not enough xxxToken");
    uint256 bonus = user.amount.mul(pool.accxxxTokenPerShareTilBonusEnd).div(1e12).sub(user.bonusDebt);
    safexxxTokenTransfer(_to, pending);
    xxxToken.lock(_to, bonus.mul(bonusLockUpBps).div(10000));
  }
  ...
  function emergencyWithdraw(uint256 _pid) public {
    ...
    // Safe xxxToken transfer function, just in case if rounding error causes pool to not have enough xxxToken.
  function safexxxTokenTransfer(address _to, uint256 _amount) internal {
    uint256 xxxTokenBal = xxxToken.balanceOf(address(this));
    if (_amount > xxxTokenBal) {
      xxxToken.transfer(_to, xxxTokenBal);
    } else {
      xxxToken.transfer(_to, _amount);
    }
  }
